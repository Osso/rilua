...
//! # Safety contract
//!
//! All heap-allocated objects (`WrappedObject`, `MarkedString`) are created via
//! `Box::into_raw` and accessed through `NonNull` pointers (`ObjectPtr`,
//! `StringPtr`). Multiple copies of these pointer wrappers can exist
//! simultaneously (they are `Copy`/`Clone`).
//!
//! The pointers remain valid because:
//! 1. GC collection only runs inside `new_obj_from_raw` and `new_string`, and
//!    only after all reachable roots have been marked via the `mark` callback.
//! 2. No VM operation dereferences a pointer across an allocation point.
//! 3. `GcHeap::drop` frees all remaining objects when the VM shuts down;
//!    no pointers are dereferenced after that.
...
impl ObjectPtr {
    ...
    fn deref(&self) -> &WrappedObject {
        // SAFETY: The pointer was created by `Box::into_raw` in `new_obj_from_raw`
        // and remains valid because GC only frees unreachable objects after a mark
        // phase. Callers never hold this reference across a GC collection point.
        unsafe { self.ptr.as_ref() }
    }

    fn deref_mut(&mut self) -> &mut WrappedObject {
        // SAFETY: Same pointer validity guarantee as `deref`. Note that
        // `ObjectPtr` is `Copy`, so `&mut self` alone does NOT prevent two
        // copies from creating aliased `&mut WrappedObject` references.
        // Exclusive access is upheld by a VM-level invariant: the instruction
        // dispatch loop never holds two `&mut` references to the same object
        // simultaneously (each instruction operates on one table at a time).
...
impl GcHeap {
    ...
    pub(super) fn collect(&mut self) {
        ...
        while !next_ptr_ref.is_null() {
            // SAFETY: `next_ptr_ref` is a `&mut *mut WrappedObject` pointing into
            // the linked list. The inner `*mut` was produced by `Box::into_raw` in
            // `new_obj_from_raw` and has not been freed yet (we free it below only
            // when unmarked). We hold `&mut self` on the GcHeap, so no other code
            // can mutate or free list nodes concurrently.
            ...
            match next_obj.color.get() {
                ...
                Color::Unmarked => {
                    ...
                    // `new_obj_from_raw`. This object is unreachable (unmarked),
                    // so no live `ObjectPtr` refers to it. Reconstituting the
                    // `Box` transfers ownership back so it can be dropped.
        ...
        for ptr in &self.strings {
            // SAFETY: The pointer was created by `Box::into_raw` in
            // `new_string`. The `MarkedString` is still alive because we only
            // free strings below, after this loop finishes.
        ...
        for ptr in strings_to_remove {
            ...
            // SAFETY: The pointer was created by `Box::into_raw` in `new_string`.
            // We just removed it from the `HashSet`, so this is the sole owner.
            // The string is unreachable (unmarked), so no live `StringPtr` in the
            // VM's value stack or globals refers to it.
...
impl Drop for GcHeap {
    fn drop(&mut self) {
        ...
        while !next_ptr.is_null() {
            // SAFETY: The pointer was created by `Box::into_raw` in
            // `new_obj_from_raw`. The GcHeap is being dropped, so the entire VM
            // is shutting down — no code will dereference any `ObjectPtr` after
            // this. Each node is visited exactly once via the linked list.
        ...
        for ptr in self.strings.drain() {
            // SAFETY: The pointer was created by `Box::into_raw` in `new_string`.
            // Same shutdown guarantee — no `StringPtr` will be dereferenced after
            // the GcHeap is dropped. `drain()` yields each pointer exactly once.
            let _boxed = unsafe { Box::from_raw(ptr.0.as_ptr()) };
        }
    }
}
...
impl Markable for WrappedObject {
    fn mark_reachable(&self) {
        if matches!(self.color.get(), Color::Unmarked) {
...
impl Markable for StringPtr {
    fn mark_reachable(&self) {
        // SAFETY: Same pointer validity guarantee as `as_str`. Marking always
        // happens *before* collection, so the `MarkedString` is still alive.
        // `Cell` allows interior mutation without `&mut`, which is needed
        // because marking traverses shared references.
