...
- `(true or 1) == true` hangs: or-shortcircuit result used directly as
  EQ operand produces a jump cycle in generated bytecode. Workaround:
...
done. Known bugs as of Phase 7a:

- **free_register assertion** (HIGH PRIORITY): `debug_assert_eq` in
  `free_register` (codegen.rs:543) panics on complex expressions. The
  assertion requires `free_reg - 1 == reg`, but complex compilation
  (table constructors, multi-assignment, nested calls) can free
  registers out of expected order. PUC-Rio's `luaK_freereg` does NOT
  assert ordering -- it silently ignores frees of registers below
  `freereg`. This blocks ALL 23 PUC-Rio test files in debug builds.
  Fix: match PUC-Rio's behavior and only decrement `freereg` when
  `reg == freereg - 1`.
...
| GC incremental | Incremental collection, `gc.lua` passes | 7b | -- |
